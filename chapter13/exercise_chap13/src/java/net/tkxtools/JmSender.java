 ...
 * 添付ファイル付きの送信も可能
 * 送信元はWindowsと前提してCP932のコード変換に対応している
 * 日本語添付ファイル名への対応に ozacc-mail library　を使用　http://sourceforge.jp/projects/spring-ext/ 
 * 
 * (C)Takashi KAWABA  
 * 
 *    2006.1.27 SMTP＿AUTH に対応するよう処理を追加した
 *    2012.5　　ポートを指定できるようにした。また日本語添付ファイル名に対応した
 * 
 */
import java.io.File;
...
public class JmSender {
    ...
    //
    /**
     * pop-before-smtp対応のコンストラクタ
     * 事前に、JmAuthenticatorでサーバーと接続しておく
     */
    ...
    /**
     * 接続 (ポートも指定）
     * 　SMTP_AUTH の場合は、session からuser, passwordが自動的に
    ...
    public	static	void	sendPBS(String smtpUser, 
    	...
    	if(!JmAuthenticate.authenticate(host, smtpUser, smtpPassword))	throw new MessagingException("Pop認証できない");
    ...
    String	pathString(String path){
    	...
    	if(e.equals(File.separator))	return	path;
	...
	/**
	 * Authenticatorクラス
	 * 　任意のユーザー名／パスワードの獲得方法を定義できる
	 *　 session の生成にこのクラスオブジェクトを使う
	 */
